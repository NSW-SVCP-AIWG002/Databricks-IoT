...
EVENTHUBS_NAMESPACE = dbutils.secrets.get(
    "eventhubs_secrets", "eventhubs-namespace")
EVENTHUBS_CONNECTION_STRING = dbutils.secrets.get(
    "eventhubs_secrets", "eventhubs-connection-string")
...
STORAGE_ACCOUNT = dbutils.secrets.get(
    "storage_secrets", "storage-account-name")
CHECKPOINT_LOCATION = (
...
def get_alert_settings():
    ...
    return (
        builtins.spark.read.format("jdbc").options(
            **_jdbc_options("alert_setting_master")).load()
...
def process_sensor_batch(batch_df, batch_id):
    ...
    print(
        f"[BATCH {batch_id}] {_ts()}アラート設定マスタ取得完了: {alert_settings_df.count()}件")
    ...
    print(
        f"[BATCH {batch_id}] {_ts()}STEP5a: Delta Lake 書込み開始 → {SILVER_TABLE}")
    ...
    try:
        ...
        print(
            f"[BATCH {batch_id}] {_ts()}STEP5a-2: mysql_df.collect() 完了 → {len(mysql_records)}件")
        ...
        if mysql_records:
            ...
            print(
                f"[BATCH {batch_id}] {_ts()}STEP5a-2: MySQL silver_sensor_data 書込み成功: {len(mysql_records)}行")
        else:
            print(
                f"[BATCH {batch_id}] {_ts()}STEP5a-2: mysql_records が空のためスキップ")
    ...
    return (
        ...
        .withColumn(
            ...
            extract_device_id_udf(
                F.col("topic"),
                F.col("message_key"),
                F.get_json_object(F.col("value").cast(
                    "string"), "$.device_id"),
            )
        )
        ...
        .select(
            ...
            F.col("parsed.defrost_heater_output_1").alias(
                "defrost_heater_output_1"),
            F.col("parsed.defrost_heater_output_2").alias(
...
def diagnose_kafka_stream():
    ...
    s3 = s2f.withColumn(
        "raw_json",
        convert_to_json_with_device_id_udf(
            F.col("value"), F.col("extracted_device_id"))
    )
    print(f"[DIAGNOSE] STEP1.6 JSON変換後（フィルタ前）: {s3.count()}件")
    s3f = s3.filter(F.col("raw_json").isNotNull() &
                    F.col("raw_json").rlike(r'^\{.*\}'))
    print(f"[DIAGNOSE] STEP1.6 JSONフィルタ後: {s3f.count()}件")

    s4 = s3f.withColumn("parsed", F.from_json(
        F.col("raw_json"), sensor_schema))
    ...
    if s3f.count() > 0:
        ...
        s1.select("topic", "message_key", F.col("value").cast(
            "string").alias("value_str")).show(3, truncate=200)
    ...
    try:
        count_df = (
            ...
            .options(
                ...
                password=_dbutils_proxy.secrets.get(
                    "my_sql_secrets", "password"),
            )
            .load()
        )
        print(
            f"[BATCH_TEST] silver_sensor_data 件数: {count_df.collect()[0]['cnt']}")
    except Exception as e:
