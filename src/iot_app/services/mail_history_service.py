...
def get_mail_type_by_id(mail_type_id: int) -> Optional[MailTypeMaster]:
    ...
    return MailTypeMaster.query.get(mail_type_id)
...
def get_default_date_range() -> tuple[date, date]:
    ...
    Returns:
        (sent_at_start, sent_at_end): 7日前00:00 〜 当日23:59 の date オブジェクト
    """
    jst = pytz.timezone('Asia/Tokyo')
    now_jst = datetime.now(jst)
    start = (now_jst - timedelta(days=7)).replace(
        hour=0, minute=0, second=0, microsecond=0
    )
    end = now_jst.replace(hour=23, minute=59, second=0, microsecond=0)
...
def get_mail_history_list(
    ...
    col = getattr(MailHistory, sort_column)
