...
# ============================================================
# Section 9: デバイス存在 & データスコープチェック
# ============================================================


@pytest.mark.unit
class TestRegisterBarChartGadgetDeviceCheck:
    """棒グラフガジェット登録 - デバイス固定モード時のデバイス存在チェック

    観点: 2.2 対象データ存在チェック
    """

    def _fixed_mode_params(self, device_id=999):
        return {
            "title": "テストガジェット",
            "device_mode": "fixed",
            "device_id": device_id,
    ...
    @patch("iot_app.services.customer_dashboard.bar_chart_service.db")
    @patch("iot_app.services.customer_dashboard.bar_chart_service.check_device_access")
    def test_fixed_mode_valid_device_proceeds_to_register(self, mock_check_device, mock_db):
        """2.2.1: デバイス固定モードで対象デバイスが存在する場合は正常に登録処理が進む"""
        from iot_app.services.customer_dashboard.bar_chart_service import register_bar_chart_gadget

        # Arrange
        mock_check_device.return_value = Mock(device_id=999)
        mock_db.session.add = Mock()
        mock_db.session.commit = Mock()
        mock_db.session.query = Mock()
        mock_db.session.query.return_value.filter.return_value.scalar.return_value = 0
        ...
        register_bar_chart_gadget(
            self._fixed_mode_params(), current_user_id=1, accessible_org_ids=[1, 2]
        )
        mock_db.session.add.assert_called_once()

    @patch("iot_app.services.customer_dashboard.bar_chart_service.check_device_access")
    def test_fixed_mode_device_not_found_raises_not_found_error(self, mock_check_device):
        """2.2.2: デバイス固定モードで対象デバイスが存在しない場合は NotFoundError が発生する"""
        from iot_app.services.customer_dashboard.bar_chart_service import register_bar_chart_gadget
        from iot_app.common.exceptions import NotFoundError

        # Arrange
        mock_check_device.return_value = None  # デバイスが見つからない
    ...
    @patch("iot_app.services.customer_dashboard.bar_chart_service.check_device_access")
    def test_fixed_mode_device_outside_scope_raises_not_found_error(self, mock_check_device):
        """2.2.3: アクセス可能スコープ外のデバイス（論理削除含む）は NotFoundError が発生する"""
        from iot_app.services.customer_dashboard.bar_chart_service import register_bar_chart_gadget
        from iot_app.common.exceptions import NotFoundError

        # Arrange
        # check_device_access はスコープ外 / 論理削除済みデバイスに対して None を返す
        mock_check_device.return_value = None

        # Act & Assert
        with pytest.raises(NotFoundError):
            register_bar_chart_gadget(
                self._fixed_mode_params(), current_user_id=1, accessible_org_ids=[1]
            )

    @patch("iot_app.services.customer_dashboard.bar_chart_service.check_device_access")
    def test_variable_mode_skips_device_check(self, mock_check_device):
        """2.2.1: デバイス可変モードの場合は check_device_access が呼ばれない"""
        from iot_app.services.customer_dashboard.bar_chart_service import register_bar_chart_gadget
        ...
        with patch("iot_app.services.customer_dashboard.bar_chart_service.db") as mock_db:
            mock_db.session.add = Mock()
            mock_db.session.commit = Mock()
            mock_db.session.query = Mock()
            mock_db.session.query.return_value.filter.return_value.scalar.return_value = 0
            ...
            # Act
            register_bar_chart_gadget(variable_params, current_user_id=1, accessible_org_ids=[1])
