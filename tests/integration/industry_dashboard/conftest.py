...
def industry_test_data(app):
    ...
    # ------------------------------------------------------------------ #
    # テストユーザーを user_master に挿入（VIEWが user_master を参照するため必須）
    # ------------------------------------------------------------------ #
    _user_type_row = db.session.execute(
        _text("SELECT user_type_id FROM user_type_master WHERE delete_flag=0 LIMIT 1")
    ).first()
    assert _user_type_row, "user_type_master にレコードがありません"

    _region_row = db.session.execute(
        _text("SELECT region_id FROM region_master LIMIT 1")
    ).first()
    _created_region = False
    if _region_row is None:
        db.session.execute(
            _text(
                "INSERT INTO region_master (region_id, region_name, time_zone) "
                "VALUES (1, '日本', 'Asia/Tokyo')"
            )
        )
        _region_id = 1
        _created_region = True
    else:
        _region_id = _region_row[0]

    _max_user_id_row = db.session.execute(
        _text("SELECT COALESCE(MAX(user_id), 0) FROM user_master")
    ).first()
    test_user_id = _max_user_id_row[0] + 1

    db.session.execute(
        _text(
            "INSERT INTO user_master "
            "(user_id, databricks_user_id, user_name, organization_id, email_address, "
            "user_type_id, region_id, address, creator, modifier) "
            "VALUES (:uid, :duid, :name, :org, :email, :utype, :region, :addr, :c, :m)"
        ),
        {
            "uid": test_user_id,
            "duid": f"test-user-{uid}",
            "name": "テストユーザー_結合テスト用",
            "org": org_accessible.organization_id,
            "email": f"test_{uid}@test.example.com",
            "utype": _user_type_row[0],
            "region": _region_id,
            "addr": "テスト住所",
            "c": _C,
            "m": _C,
    ...
    # ------------------------------------------------------------------ #
    # inject_test_user の user_id / organization_id をテストユーザーで override
    # ------------------------------------------------------------------ #
    accessible_org_id = org_accessible.organization_id

    def _override_org():
        if hasattr(g, "current_user"):
            g.current_user.user_id = test_user_id
    ...
    db.session.execute(
        _text("DELETE FROM user_master WHERE user_id = :uid"),
        {"uid": test_user_id},
    )
    ...
    if _created_region:
        db.session.execute(
            _text("DELETE FROM region_master WHERE region_id = :rid"),
            {"rid": _region_id},
        )
