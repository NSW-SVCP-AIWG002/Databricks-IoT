...
バッチジョブは、CRON列で定義される設定時間に従って起動するバッチ処理です。
...
Deltaテーブルクリーンアップジョブは、Deltaテーブル上で保持期間の経過したデータを定期削除するバッチジョブです。`silver_sensor_data`、`gold_sensor_data_hourly_summary`、`gold_sensor_data_daily_summary`、`gold_sensor_data_monthly_summary`テーブルで保持期間の経過したデータを対象に、データ削除を行います。

メール通知キュークリーンアップジョブは、OLTP DBの`email_notification_queue`テーブルを対象に、滞留した保持期間の経過したデータを定期削除するバッチジョブです。
...
| テーブル                 | 保持期間 | 削除対象            | 削除方式 |
| ------------------------ | -------- | ------------------- | -------- |
| email_notification_queue | 15分間   | PROCESSINGレコード  | DELETE   |
...
| 日付       | 版数 | 変更内容 | 担当者       |
| ---------- | ---- | -------- | ------------ |
| 2026-04-03 | 1.0  | 初版作成 | Kei Sugiyama |
