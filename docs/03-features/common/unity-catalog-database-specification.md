...
- [Unity Catalog データベース設計書](#unity-catalog-データベース設計書)
  ...
  - [動的ビュー](#動的ビュー)
    - [動的ビュー一覧](#動的ビュー一覧)
      ...
      - [5. センサーデータ日本語名ビュー（センサーデータ）](#5-センサーデータ日本語名ビューセンサーデータ)
      - [6. センサーデータ日次サマリ日本語名ビュー（日次集計）](#6-センサーデータ日次サマリ日本語名ビュー日次集計)
      - [7. センサーデータ月次サマリ日本語名ビュー（月次集計）](#7-センサーデータ月次サマリ日本語名ビュー月次集計)
      - [8. センサーデータ年次サマリ日本語名ビュー（年次集計）](#8-センサーデータ年次サマリ日本語名ビュー年次集計)
...
│   ├── センサーデータ                               # センサーデータ日本語名ビュー（Genie向け）
│   ├── 日次集計                                    # センサーデータ日次サマリ日本語名ビュー（Genie向け）
│   ├── 月次集計                                    # センサーデータ月次サマリ日本語名ビュー（Genie向け）
│   └── 年次集計                                    # センサーデータ年次サマリ日本語名ビュー（Genie向け）
│
...
| 5   | センサーデータ       | センサーデータ日本語名ビュー | Genie向け日本語名ビュー          |
| 6   | 日次集計             | 日次サマリ日本語名ビュー     | Genie向け日本語名ビュー          |
| 7   | 月次集計             | 月次サマリ日本語名ビュー     | Genie向け日本語名ビュー          |
| 8   | 年次集計             | 年次サマリ日本語名ビュー     | Genie向け日本語名ビュー          |
...
| 2   | ts           | チェックポイントID   | STRING   | NOT NULL | 〇  |     | タイムスタンプ（チェックポイントの版管理） |
| 3   | parent_ts    | 親チェックポイントID | STRING   | NULL     |     |     | 親チェックポイントのID                     |
...
- 使われ方の具体例は以下の通り
  - 保存（各ノード実行後に自動）
        ユーザー：「昨日の冷凍庫の温度は？」（thread_id = "abc-123"）
            Planner実行 → check_point_dataに保存（ts="t1"）
            GenieAPI実行 → check_point_dataに保存（ts="t2", parent_ts="t1"）
            GraphAPI実行 → check_point_dataに保存（ts="t3", parent_ts="t2"）
            LLM実行     → check_point_dataに保存（ts="t4", parent_ts="t3"）
...
#### 5. センサーデータ日本語名ビュー（センサーデータ）

```sql
-- 日本語名ビュー（参照先は英語名ビュー＝Row Access Policy適用済み）
CREATE OR REPLACE VIEW iot_catalog.views.`センサーデータ` AS
SELECT * FROM iot_catalog.views.sensor_data_view;
```

#### 6. センサーデータ日次サマリ日本語名ビュー（日次集計）

```sql
-- 日本語名ビュー（参照先は英語名ビュー＝Row Access Policy適用済み）
CREATE OR REPLACE VIEW iot_catalog.views.`日次集計` AS
SELECT * FROM iot_catalog.views.daily_summary_view;
```

#### 7. センサーデータ月次サマリ日本語名ビュー（月次集計）

```sql
-- 日本語名ビュー（参照先は英語名ビュー＝Row Access Policy適用済み）
CREATE OR REPLACE VIEW iot_catalog.views.`月次集計` AS
SELECT * FROM iot_catalog.views.monthly_summary_view;
```

#### 8. センサーデータ年次サマリ日本語名ビュー（年次集計）

```sql
-- 日本語名ビュー（参照先は英語名ビュー＝Row Access Policy適用済み）
CREATE OR REPLACE VIEW iot_catalog.views.`年次集計` AS
SELECT * FROM iot_catalog.views.yearly_summary_view;
```
