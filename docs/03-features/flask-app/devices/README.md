...
| カラム名                    | 論理名                 | データ型     | 必須 | 備考                                                             |
| --------------------------- | ---------------------- | ------------ | ---- | ---------------------------------------------------------------- |
| device_id                   | デバイスID             | INT          | ○    | 主キー                                                           |
| device_uuid               | デバイスID             | VARCHAR(128) | ○    | Azure IoT HubsのデバイスID、AWS IoT CoreのクライアントIDと同じID |
...
| sim_id                      | SIMID                  | VARCHAR(20)  |      |                                                                  |
| mac_address                 | MACアドレス            | VARCHAR(17)  |      |                                                                  |
| organization_id             | 組織ID                 | INT          | ○    | 外部キー: organization_master.organization_id                    |
...
| カラム名    | 論理名     | データ型     | 必須 | 備考                                        |
| ----------- | ---------- | ------------ | ---- | ------------------------------------------- |
| device_id   | デバイスID | INT          | ○    | 主キーかつ外部キー: device_master.device_id |
| status      | ステータス | INT          | ○    | 0：未接続、1：接続済み                      |
| delete_flag | 削除フラグ | BOOLEAN      | ○    | True：論理削除済、False：その他             |
| create_date | 作成日時   | DATETIME     | ○    | レコードの作成時刻                          |
| creator     | 作成者     | VARCHAR(100) | ○    | レコード作成者のユーザID                    |
| update_date | 更新日時   | DATETIME     | ○    | レコードの更新時刻                          |
| modifier    | 更新者     | VARCHAR(100) | ○    | レコード更新者のユーザID                    |
...
**機能制限**
- サービス利用者に対して、登録ボタン、更新ボタン、削除ボタンは非表示とする
