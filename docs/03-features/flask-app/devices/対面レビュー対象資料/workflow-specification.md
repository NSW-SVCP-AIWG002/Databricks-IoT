...
| 更新実行         | フォーム送信    | `POST /admin/devices/<device_id>/update` | フォームデータ                                                                                                              | リダイレクト → `GET /admin/devices` | フォーム再表示（エラーメッセージ付き） |
| 削除実行         | フォーム送信    | `POST /admin/devices/<device_id>/delete` | device_id                                                                                                                   | リダイレクト → `GET /admin/devices` | エラーメッセージ表示                   |
...
```mermaid
flowchart TD
    Start([検索ボタンクリック]) --> Convert[検索条件をクエリパラメータに変換<br>page: 1（リセット）]
...
WHERE
  d.delete_flag = 0
  AND CASE WHEN :device_id IS NULL THEN TRUE ELSE d.device_id LIKE CONCAT('%', :device_id, '%') END
...
```mermaid
flowchart TD
    Start([ページ番号ボタンクリック]) --> Convert[検索条件をクエリパラメータに変換]
    Convert --> Scope[データスコープ制限を適用]
    Scope --> Count[検索結果件数取得DBクエリ実行<br>検索条件を適用]
    Count --> Query[検索結果DBクエリ実行<br>検索条件を適用]
    Query --> CheckDB{DBクエリ結果}
    ...
    CheckDB -->|失敗| Error500[Jinja2テンプレートレンダリング<br>ページ番号は強制的に1へ]
...
flowchart TD
    ...
    LoadOrgs --> LoadCategory[デバイス種別マスタを取得<br>SELECT * FROM device_type_master]
    LoadCategory --> Template[登録モーダルをレンダリング<br>render_template<br>admin/devices/form.html]
    Template --> End([モーダル表示])
...
flowchart TD
    ...
    FrontValidate --> DupCheck[デバイスID重複チェック<br>SELECT * FROM device_master<br>WHERE device_id = :device_id]
    ...
    DupCheck2 --> DupResult2{重複あり？}
    DupResult2 --> |重複なし| Insert[デバイス登録<br>INSERT INTO device_masters]
    ...
    CheckUC --> |成功|Flash[フラッシュメッセージ設定<br>デバイスを登録しました]
...
flowchart TD
    ...
    CheckPerm -->|権限OK| LoadDevice[デバイス情報を取得<br>SELECT * FROM device_master<br>WHERE device_id = :device_id]
    LoadDevice --> CheckData{データ存在?}

    CheckData -->|なし| Error404[404エラーページ]

    CheckData -->|あり| ScopeCheck[データスコープチェック]
    ScopeCheck --> CheckScope{スコープOK?}
    CheckScope -->|NG| Error404

    CheckScope -->|OK| LoadOrgs[デバイス登録内容を取得]
...
flowchart TD
    ...
    ConfirmResult -->|OK| GetOriginal[元データ取得<br>SELECT * FROM device_master<br>WHERE device_id = :device_id]
    
    GetOriginal --> CheckExists{デバイス存在?}
    CheckExists -->|存在しない| NotFoundError[エラー表示<br>指定されたデバイスが見つかりません]
    NotFoundError --> ValidEnd
    
    CheckExists -->|存在する| Delete[デバイス削除<br>DELETE FROM device_master<br>WHERE device_id = :device_id]
    
    Delete --> CheckDB{DB操作結果}
    
    CheckDB -->|成功| DeleteUC[UnityCatalog.<br>device_masterから<br>デバイス削除<br>DELETE FROM<br>device_master<br>WHERE device_id = :device_id]
    DeleteUC --> CheckUC{UnityCatalog<br>操作結果}
    CheckUC -->|成功| Flash[フラッシュメッセージ設定<br>デバイスを削除しました]
    Flash --> Redirect[一覧画面へリダイレクト<br>redirect url_for admin.list_devices]
    
    CheckDB -->|失敗| Rollback[DB.device_master<br>元データを復元<br>INSERT INTO device_master]
    CheckUC -->|失敗| RollbackUC[UnityCatalog.<br>device_master<br>元データを復元<br>INSERT INTO device_master]
    RollbackUC --> Rollback
    Rollback --> Error500[エラーメッセージ表示]
...
def export_devices_csv(devices):
    ...
    for device in devices:
        writer.writerow([
            ...
            device.organization.organization_name if device.organization else ''
        ])
...
- device_master.device_name: INDEX - 名前検索高速化
