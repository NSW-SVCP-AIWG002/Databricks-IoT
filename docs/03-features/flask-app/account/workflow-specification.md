...
**処理概要**:
1. ログインユーザーのuser_idを取得（リバースプロキシヘッダ `X-Databricks-User-Id`）
...
リバースプロキシヘッダから認証情報を取得します。

**処理内容:**
- ヘッダ `X-Databricks-User-Id` からユーザーIDを取得
- ユーザーIDが存在しない場合はログイン画面へリダイレクト
...
def language_settings():
    ...
    current_user_id = request.headers.get('X-Databricks-User-Id')
    ...
    try:
        ...
        form.language_code.choices = [
            (lang['language_code'], lang['language_code']) for lang in languages
        ]
        ...
        logging.error(f'言語設定画面表示エラー: {str(e)}')
...
def update_language_settings():
    ...
    try:
        ...
        if not form.validate_on_submit():
            ...
            return render_template('account/language.html', form=form, languages=languages), 400
        ...
        query_update = """
            ...
            WHERE user_id = %s AND delete_flag = 0
        """
        cursor.execute(query_update, (language_code, current_user_id, current_user_id))
...
def profile():
    ...
    try:
        ...
        query = """
            ...
            FROM user_master u
            LEFT JOIN organization_master o ON u.organization_id = o.organization_id
            WHERE u.user_id = %s AND u.delete_flag = 0
        """
        ...
        logging.error(f'ユーザ情報参照画面表示エラー: {str(e)}')
        flash('画面表示に失敗しました', 'error')
        return redirect(url_for('dashboard.index'))
    finally:
        cursor.close()
...
**Databricks標準認証**:
- リバースプロキシが付与するヘッダ情報からユーザーを識別
- `X-Databricks-User-Id`: ユーザーID
- `X-Databricks-Access-Token`: アクセストークン
