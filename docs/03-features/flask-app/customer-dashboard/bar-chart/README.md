...
- [顧客作成ダッシュボード棒グラフ機能](#顧客作成ダッシュボード棒グラフ機能)
  - [📑 目次](#-目次)
  - [概要](#概要)
  - [機能一覧](#機能一覧)
  - [画面一覧](#画面一覧)
...
IoTデバイスから収集したセンサーデータを棒グラフで可視化する機能です。顧客作成ダッシュボード画面に埋め込んで表示します。シルバー層（構造化データ）またはゴールド層（集計データ）のデータを時間単位（時/日/週/月）で表示し、センサー値の推移を確認可能です。
...
| 機能ID | 機能名 | 概要 |
|--------|--------|------|
| FR-006-8 | ガジェット表示 | 選択した項目・期間のデータを棒グラフで表示 |
| FR-006-9 | ガジェット登録 | 棒グラフガジェットの新規登録 |
...
|--------|--------|---------|---------|------|
| CDS-001 | 顧客作成ダッシュボード表示画面 | `/analysis/customer-dashboard` | 画面 | 顧客作成ダッシュボード画面に棒グラフガジェットを埋め込んで表示 |
| CDS-006 | 棒グラフガジェット登録モーダル | `/analysis/customer-dashboard/gadgets/bar-chart/create` | モーダル | 棒グラフガジェットを登録 |
...
| 4 | ガジェット登録実行 | `/analysis/customer-dashboard/gadgets/bar-chart/register` | POST | 棒グラフガジェット登録処理 | リダイレクト (302) |
