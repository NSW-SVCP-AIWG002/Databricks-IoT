...
| 登録ボタン押下 | ボタンクリック | `POST /analysis/customer-dashboard/gadgets/stacked-bar-chart/register` | `title, device_mode, device_id, group_id, summary_method_id, measurement_item_ids, gadget_size` | JSON (AJAX) | エラートースト表示 |
...
WHERE
  ...
  AND u.delete_flag = FALSE
  AND device_id = :device_id
  AND measurement_item_id IN :measurement_item_ids
...
```python
# services/customer_dashboard/stacked_bar_chart.py
INTERVAL_MINUTES = {
...
def format_stacked_bar_chart_data(rows, display_unit, measurement_items, interval='10min', summary_method_id=1):
    ...
    if display_unit == 'hour':
        ...
        for item in measurement_items:
            ...
            for row in rows:
                if row['measurement_item_id'] == item_id:
...
```python
# views/analysis/customer_dashboard/stacked_bar_chart.py
def handle_gadget_data(gadget_uuid):
    ...
    try:
        ...
        chart_data = format_stacked_bar_chart_data(rows, display_unit, measurement_items, interval, summary_method_id)
...
```mermaid
flowchart TD
    Start([登録画面ボタンクリック]) --> Validate[バリデーション<br>ガジェット種別]
    ...
    CheckSummaryMethodQuery -->|成功| Response[積み上げ棒グラフガジェット登録モーダル<br>HTMLレスポンス返却]
...
def handle_gadget_create(gadget_type):
    ...
    try:
        context = get_stacked_bar_chart_create_context(g.current_user.user_id)
    except Exception as e:
        logger.error(f'積み上げ棒グラフ登録モーダル表示エラー: {str(e)}')
        abort(500)

    form = StackedBarChartGadgetForm()
    ...
    return render_template(
        'analysis/customer_dashboard/gadgets/modals/stacked_bar_chart.html',
        form=form,
...
def handle_gadget_register(gadget_type):
    ...
    if not form.validate_on_submit():
        try:
            ...
            logger.error(f'積み上げ棒グラフガジェット登録コンテキスト取得エラー: {str(e)}')
            abort(500)
        return render_template(
            'analysis/customer_dashboard/gadgets/modals/stacked_bar_chart.html',
            form=form,
    ...
    try:
        register_stacked_bar_chart_gadget(form_data, g.current_user.user_id, g.current_user.user_id)
        return jsonify({'message': msg_created('ガジェット')})
    except NotFoundError:
        abort(404)
    except Exception as e:
        logger.error(f'積み上げ棒グラフガジェット登録エラー: {str(e)}', exc_info=True)
        abort(500)
```
...
def handle_gadget_csv_export(gadget_uuid):
    ...
    try:
        ...
        csv_content = generate_stacked_bar_chart_csv(
        ...
        logger.error(f'積み上げ棒グラフCSVエクスポートエラー: gadget_uuid={gadget_uuid}, error={str(e)}', exc_info=True)
        abort(500)
```
