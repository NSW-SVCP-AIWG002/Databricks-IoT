...
|----|---------|---------------|---------|------|---------------|------|
| 1 | メール通知履歴一覧表示（初期表示） | `/notice/mail-history` | GET | メール通知履歴一覧表示（検索条件なし） | HTML | ページング対応 |
| 2 | メール通知履歴検索 | `/notice/mail-history` | POST | メール通知履歴検索（検索条件あり） | HTML | 検索条件をセッションに保存 |
| 3 | メール通知履歴詳細表示 | `/notice/mail-history/<mail_history_id>` | GET | メール通知履歴詳細表示（モーダル） | HTML（パーシャル） | Jinja2パーシャルテンプレート |
...
flowchart TD
    ...
    ValidCheck -->|OK| Session[検索条件をセッションに保存<br>mail_types, keyword<br>sent_at_start, sent_at_end<br>page=1, sort_id=3, order=desc]
    Session --> Query[DBクエリ実行<br>SELECT * FROM mail_history<br>WHERE organization_id=現在の組織ID<br>AND mail_type IN (mail_types)<br>AND (subject LIKE '%keyword%' OR body LIKE '%keyword%')<br>AND sent_at BETWEEN sent_at_start AND sent_at_end<br>ORDER BY sent_at DESC, mail_history_id ASC<br>LIMIT 25 OFFSET 0]
    Query --> CheckDB{DBクエリ<br>結果}
...
ソート処理は、セッションに保存された検索条件を保持したまま、ソート条件を変更して検索処理を実行します。
...
def mail_history_list():
    # セッションから検索条件を取得
    search_params = session.get('mail_history_search', {})
    ...
    session['mail_history_search'] = search_params
...
ページング処理は、セッションに保存された検索条件とソート条件を保持したまま、ページ番号を変更して検索処理を実行します。
...
データベースへのリクエスト時は、パラメータライズドクエリ（プリペアドステートメント）を使用し、SQLインジェクションを防止します。

**実装方式:**
- ✅ SQLAlchemyのORMまたはパラメータライズドクエリを使用
- ✅ 文字列結合によるSQL生成は禁止
- ✅ ORDER BY句等、動的に値を埋め込む場合は事前にホワイトリスト検証を実施
- ✅ ユーザー入力値は必ずエスケープまたはバインド変数を使用

**実装例:**
```python
# ✅ 良い例: パラメータライズドクエリ
mail_histories = MailHistory.query.filter(
    MailHistory.organization_id == organization_id,
    MailHistory.mail_type.in_(mail_types)  # バインド変数
).all()

# ❌ 悪い例: 文字列結合（SQLインジェクションのリスク）
query = f"SELECT * FROM mail_history WHERE organization_id = '{organization_id}'"
```
