...
- リクエストID
- ユーザーID（操作者）
- 操作種別（メール通知履歴一覧表示、詳細表示等）
- 対象リソースID（mail_history_id等）
- 処理結果（成功/失敗）
- エラー種別（認証エラー、DBエラー等）
...
@notice_bp.route('/mail-history', methods=['GET'])
def mail_history_list():
    logger.info(f"メール通知履歴一覧表示開始 - 操作者: {current_user.user_id}")

    try:
        mail_histories = get_mail_histories()
        logger.info(f"メール通知履歴一覧表示成功 - 操作者: {current_user.user_id}, 件数: {len(mail_histories)}")
        return render_template('mail-history/list.html', mail_histories=mail_histories)

    except Exception as e:
        logger.error(f"メール通知履歴一覧表示失敗（サーバーエラー） - 操作者: {current_user.user_id}, エラー: {type(e).__name__}")
        abort(500)
```
