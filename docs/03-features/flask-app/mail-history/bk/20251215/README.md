...
```mermaid
erDiagram
    MAIL_HISTORY ||--o| USERS : "送信先ユーザー"
    MAIL_HISTORY ||--o| ORGANIZATIONS : "関連組織"
...
| No | カラム名 | 論理名 | データ型 | NULL | 説明 |
|----|---------|--------|---------|------|------|
| 1 | mail_history_id | メール送信履歴ID | VARCHAR(36) | NOT NULL | 主キー、UUID |
...
| 8 | user_id | 関連ユーザーID | VARCHAR(36) | NULL | 関連するユーザーID（外部キー） |
| 9 | organization_id | 関連組織ID | VARCHAR(36) | NULL | 関連する組織ID（外部キー、データスコープ制限用） |
| 10 | created_at | 作成日時 | DATETIME | NOT NULL | レコード作成日時 |
...
| 2 | users | ユーザーマスタ | SELECT | 関連ユーザー情報の取得 |
| 3 | organizations | 組織マスタ | SELECT | 関連組織情報の取得 |
