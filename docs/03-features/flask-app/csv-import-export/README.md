...
| 所属組織 | device_master.organization_id | int | ○ | - | organization_master.organization_nameをキーにorganization_idを参照 |
| 証明書期限 | device_master.certificate_expiration_date | date | - | - | - |

**CSVサンプル:**
```csv
操作列,デバイスID,デバイス名,デバイス種別,モデル情報,SIMID,MACアドレス,設置場所,所属組織,証明書期限
R,DEV-001,温度センサー1号機,センサー,MODEL-A100,12345678901234567890,AA:BB:CC:DD:EE:FF,本社ビル1階 サーバールーム,A株式会社,2025-12-31
R,DEV-002,温度センサー2号機,センサー,MODEL-A200,12345678901234567891,AA:BB:CC:DD:EE:FG,本社ビル2階 サーバールーム,A株式会社,2025-12-31
```
...
| 所属 | user_master.organization_id | int | ○ | - | organization_master.organization_nameをキーにorganization_idを参照 |
...
**CSVサンプル:**
```csv
操作列,ユーザーID,ユーザー名,メールアドレス,所属,ユーザー種別,地域,住所,アラート通知メール受信設定,システム通知メール受信設定
R,1,山田太郎,yamada@example.com,組織A,管理者,東京都,東京都渋谷区...,true,true
R,2,佐藤花子,sato@example.com,組織A,管理者,東京都,東京都渋谷区...,true,true
```
...
| 所属組織 | organization_closure.parent_organization_id | int | ○ | - | organization_master.organization_nameをキーにparent_organization_idを参照 |
...
**CSVサンプル:**
```csv
操作列,組織ID,組織名,組織種別,所属組織,住所,電話番号,FAX,担当者名,契約状態,契約開始日,契約終了日
R,1,組織A,販売会社,NSW株式会社,東京都渋谷区...,03-1234-5678,03-1234-5679,山田太郎,契約中,2025-01-01,2026-12-31
R,2,組織B,販売会社,NSW株式会社,大阪府大阪市...,06-1234-5678,06-1234-5679,佐藤花子,契約中,2025-01-01,2026-12-31
```
...
| デバイス名 | alert_setting_master.device_id | int | ○ | - | device_master.device_nameをキーにdevice_idを参照 |
...
| アラート復旧条件_閾値 | alert_setting_master.alert_recovery_threshold | double | ○ | - | - |
...
**CSVサンプル:**
```csv
操作列,アラートID,アラート名,デバイス名,アラート発生条件_測定項目,アラート発生条件_比較演算子,アラート発生条件_閾値,アラート復旧条件_測定項目,アラート復旧条件_比較演算子,アラート復旧条件_閾値,判定時間,アラートレベル,アラート通知,メール送信
R,1,温度異常,DEV-001,temperature,>,30.0,temperature,<=,30.0,5,Warning,true,false
R,2,湿度異常,DEV-001,humidity,>,50.0,humidity,<=,50.0,5,Warning,true,false
```
