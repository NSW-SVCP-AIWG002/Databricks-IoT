...
| 7 | アラート設定削除実行 | `/alert/alert-setting/<alert_uuid>/delete` | POST | アラート設定削除処理 | リダイレクト (302) | 成功時: 一覧へ |
...
flowchart TD
    ...
    Scope --> GetMaster[検索条件用マスタデータ取得<br>organization_master<br>alert_level_master<br>データスコープ制限適用]
    ...
    CheckDB -->|失敗| Error500[500エラーポップアップモーダル表示]
    ...
    CheckDB2 -->|失敗| Error500[500エラーポップアップモーダル表示]
    Error500 --> End
...
if 'page' in request.args:
    ...
    alert_name = request.cookies.get('alert_name', '')
    organization_name = request.cookies.get('organization_name', 'すべて')
    device_name = request.cookies.get('device_name', '')
    alert_level_id = request.cookies.get('alert_level_id', 'すべて')
    alert_notification_flag = request.cookies.get('alert_notification_flag', 'すべて')
    alert_email_flag = request.cookies.get('alert_email_flag', 'すべて')
    page = request.args.get('page', 1, type=int)  # クエリパラメータから取得
    per_page = request.cookies.get('per_page', 25, type=int)
    sort_by = request.cookies.get('sort_by', 'alert_id')
...
| 403 | 権限エラー | 403エラーポップアップモーダル表示 | この操作を実行する権限がありません |
| 500 | データベースエラー | 500エラーポップアップモーダル表示 | データの取得に失敗しました |
---
...
```mermaid
flowchart TD
    Start([検索ボタンクリック<br>フォーム送信]) --> Validate[サーバーサイド<br>バリデーション<br>WTForms検証]
...
**トリガー:** アラート設定登録ボタンクリック

**前提条件:**
- ユーザーがアラート設定登録権限を持っている

##### 処理フロー

```mermaid
flowchart TD
    Start([メイン画面上の登録ボタンを<br>クリック]) --> CheckPerm{権限チェック}

    CheckPerm -->|権限OK| API[デバイスマスタ、測定項目マスタ、<br>アラートレベルマスタを取得<br>データスコープ制限適用]
    API --> Config[比較演算子、判定時間を<br>設定ファイルから取得]
    Config --> Check{DBクエリ結果}

    Check -->|失敗| Error[500エラーポップアップモーダル表示]
    Error --> End
...
```mermaid
flowchart TD
    Start([登録モーダル上の<br>登録ボタンクリック]) --> Validate[サーバーサイド<br>バリデーション実行<br>WTForms検証]
    ...
    ConfirmModal --> |登録確認モーダル上の<br>登録ボタン押下| CheckAuth{権限チェック}
    CheckAuth --> |権限なし|Error403[403エラーポップアップモーダル表示]
    Error403 --> End

    CheckAuth --> |権限OK| TxStart[トランザクション開始]
...
```mermaid
flowchart TD
    Start([メイン画面データテーブル上の<br>更新ボタンクリック]) --> Permission[権限チェック]
    Permission --> CheckPerm{権限OK?}

    CheckPerm -->|権限OK| GetID[アラートUUIDを取得<br>SELECT * FROM<br> alert_setting_master<br>WHERE alert_uuid = :uuid<br>AND データスコープ制限]
    GetID --> GetMaster[デバイスマスタ、測定項目マスタ、<br>アラートレベルマスタを取得<br>データスコープ制限適用]
    GetMaster --> GetConfig[比較演算子、判定時間を<br>設定ファイルから取得]
    GetConfig --> Check{DBクエリ結果}

    Check -->|データなし| Error404[404エラーポップアップモーダル表示]
    Error404 --> End
...
```mermaid
flowchart TD
    Start([更新モーダル上の<br>更新ボタンクリック]) --> Validate[サーバーサイド<br>バリデーション]
    Validate --> ValidCheck{バリデーション結果}
    ...
    ConfirmModal -->|更新確認モーダル上の<br>更新ボタン押下| CheckParam{権限チェック}

    CheckParam --> |権限なし| Error403[403エラーポップアップモーダル表示]
    Error403 --> End

    CheckParam --> |権限OK| TxStart[トランザクション開始<br>UPDATE<br> alert_setting_master<br>SET ... <br>WHERE alert_uuid = :uuid<br>AND データスコープ制限]
    TxStart --> CheckDB{DBクエリ結果}

    CheckDB -->|0件更新| Error404[404エラーポップアップモーダル表示]
    Error404 --> End
...
**トリガー:** 一覧画面のチェックボックスで選択後、削除ボタンクリック

#### 処理フロー

```mermaid
flowchart TD
    Start([メイン画面上の<br>削除ボタンクリック]) --> UserAction{削除確認モーダルを表示}

    UserAction -->|キャンセルボタン押下| Cancel[確認モーダルを閉じる]
    Cancel --> End

    UserAction -->|削除ボタン押下| CheckAuth{権限チェック}
    
    CheckAuth -->|権限なし| Error403[403エラーポップアップモーダル表示]
    Error403 --> End

    CheckAuth -->|権限OK| TxStart[トランザクション開始<br>UPDATE<br> alert_setting_master<br>SET delete_flag = TRUE<br>WHERE alert_uuid IN :uuids<br>AND データスコープ制限]
    TxStart --> CheckDB{DBクエリ結果}
...
| ルート | エンドポイント | 詳細 |
|-------|---------------|------|
| アラート設定削除実行 | `POST /alert/alert-setting/<alert_uuid>/delete` | 論理削除（delete_flag=TRUE） |

**パスパラメータ**: `alert_uuid` - 対象アラートのUUID

**注**: 複数選択削除の場合は、アラートUUIDをカンマ区切りで送信し、サーバー側で分割処理
...
```mermaid
flowchart TD
    Start([参照ボタンクリック]) --> GetID[アラート設定を取得<br>SELECT * FROM<br> alert_setting_master<br>JOIN device_master<br>JOIN measurement_item_master<br>JOIN alert_level_master<br>WHERE alert_uuid = :uuid<br>AND データスコープ制限]
    GetID --> Check{DBクエリ結果}
...
```mermaid
flowchart TD
    Start([エクスポートボタン<br>クリック]) --> Permission[権限チェック<br>システム保守者、<br>管理者、販社ユーザのみ]
    Permission --> CheckPerm{権限OK?}

    CheckPerm -->|権限なし| Error403[403エラーポップアップモーダル表示]
    Error403 --> End
...
@alert_settings_bp.route('/alert/alert-setting/<alert_uuid>/delete', methods=['POST'])
@require_data_scope
def delete_alert_setting(alert_uuid):
    # 削除処理
    pass
```
...
- judgment_time: 1,5,10,15,30,60のいずれか、必須
