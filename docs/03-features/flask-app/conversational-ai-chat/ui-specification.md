...
| (3.1.3.2) | SQLクエリ表示                | sql_query             | Label      | O   | -    | -      | -        | 生成されたSQL                                                        | 折りたたみ可                                                                                                         |
...
│    │ - 最低温度: -19.8℃                                   │    │
│    |                                                      |    │
│    | (3.1.3.2) SQLクエリ表示（折りたたみ）                  |    |
│    │ [▶ SQLクエリを非表示]                                 │    │
│    │ ┌────────────────────────────────────────────────┐   │    │
│    │ │ SELECT AVG(internal_temp_freezer_1) as avg_temp│   │    │
│    │ │ FROM sensor_data_view                          │   │    │
│    │ │ WHERE DATE(event_timestamp) = '2026-02-03'     │   │    │
│    │ └────────────────────────────────────────────────┘   │    │
│    |                                                      |    │
│    |  (3.1.3.3)                                           |    │
|    │ ┌────────────────────────────────────────────────┐   │    │
...
**3-1-3-2: AIレスポンスメッセージ（SQLクエリ表示）**

- 概要: 入力されたユーザメッセージをもとにAIが処理を行い、その結果として返却されたSQLクエリの内容を表示する
- 初期状態では「▼SQLクエリを表示」とする
- 「▼SQLクエリを表示」リンクを押下することでSQLクエリを表示する
- 「▼SQLクエリを表示」リンクを押下すると、リンク内容が「▶SQLクエリを非表示」に変化する
- 「▶SQLクエリを非表示」リンクを押下すると、SQLクエリを非表示にする
