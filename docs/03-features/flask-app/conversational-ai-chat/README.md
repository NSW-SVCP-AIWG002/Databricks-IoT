...
Databricks Genieが英語の物理名テーブルを認識しやすくするため、日本語論理名の動的ビューを作成します。

| 動的ビュー名（日本語） | 参照先テーブル       | 用途                      |
| ---------------------- | -------------------- | ------------------------- |
| センサーデータ         | silver_sensor_data   | Genieでの日本語クエリ対応 |
| 日次集計               | daily_summary_view   | 日次データ問い合わせ      |
| 月次集計               | monthly_summary_view | 月次データ問い合わせ      |
| 年次集計               | yearly_summary_view  | 年次データ問い合わせ      |
