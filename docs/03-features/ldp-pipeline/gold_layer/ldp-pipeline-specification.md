...
OLTP DBのgold_sensor_data_hourly_summary、gold_sensor_data_daily_summary、gold_sensor_data_monthly_summaryテーブルの定義については、アプリケーションデータベース仕様書を参照のこと。
...
時次集計処理の集計対象データは、iot_catalog.silver.silver_sensor_dataテーブルに保持されているデータのうち、event_timestampの値が
任意の日時の00分00秒から、同日時の59分59秒までのものを一まとめとして集計する。

例）event_timestampが2025/04/01 3:00:00 ~ 2025/04/01 3:59:59
...
日次集計処理の集計対象データは、iot_catalog.silver.silver_sensor_dataテーブルに保持されているデータのうち、event_timestampの値が
任意の日付の00時00分00秒から、同日付の23時59分59秒までのものを一まとめとして集計する。

例）event_timestampが2025/04/01 00:00:00 ~ 2025/04/01 23:59:59
...
月次集計処理の集計対象データは、iot_catalog.silver.silver_sensor_dataテーブルに保持されているデータのうち、event_timestampの値が
任意の年月の1日00時00分00秒から、同年月の末日23時59分59秒までのものを一まとめとして集計する。

例）event_timestampが2025/04/01 00:00:00 ~ 2025/04/30 23:59:59
...
VALUES
    ...
    (9, 'SUM',    '合計',         FALSE, CURRENT_TIMESTAMP(), -999, CURRENT_TIMESTAMP(), -999);
```
...
| 入力データ         | 集約キー                                                  | 出力                                                  |
| ------------------ | --------------------------------------------------------- | ----------------------------------------------------- |
| silver_sensor_data | device_id, organization_id, event_timestamp, summary_item | マスタで有効な集約方法（delete_flag=FALSE）のレコード |
...
時次・日次・月次集計処理は構造が共通（約90%）のため、パラメータ化された共通関数として実装します。
...
| 統計値計算             | 共通      | AVG/MAX/MIN/P25/MEDIAN/P75/STDDEV/P95/SUM                                                   |
| マスタテーブル結合     | 共通      | gold_summary_method_masterとのINNER JOIN                                                    |
| MERGE処理              | 共通      | upsert処理                                                                                  |
| 集約キー               | **差分**  | event_timestamp / event_date / colecction_datetime / collecton_date / collection_year_month |
| 出力テーブル           | **差分**  | hourly / daily / monthly                                                                    |
| WHERE条件              | **差分**  | 日付条件                                                                                    |
...
def run_aggregation(period: AggregationPeriod, target_filter: str):
    ...
    Args:
        period: 集計期間（HOURLY/DAILY/MONTHLY）
...
> **月初判定の責務分担:**
> 月次集計の起動判断（月初=1日か）は**スケジューラは行わない**。
> パイプラインは `run_aggregation()` を受動的に呼び出し、かつ、日付判定ロジックを内包する。
> スケジューラは毎日に時次・日次集計を起動し、当日が月初であれば月次集計も追加で起動する。
...
- [最適化バッチ設計書](../../batch/optimization/job-specification.md)
