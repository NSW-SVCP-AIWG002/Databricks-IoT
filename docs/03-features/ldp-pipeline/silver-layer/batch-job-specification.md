...
| タイムアウト     | 5分                                            |
| リトライポリシー | 失敗時リトライなし（次回スケジュールで再実行） |
...
MAX_BATCH_SIZE = 100
MAX_RETRY_COUNT = 3
RETRY_INTERVALS = [1, 2, 4]  # 指数バックオフ（秒）
...
| リトライ間隔     | 指数バックオフ（1秒、2秒、4秒） | 送信失敗時の待機時間                   |
...
| VACUUM保持   | 168時間（7日）              | 削除ファイルの物理保持期間             |
...
def optimize_silver_tables():
    ...
    tables = [
        ...
        "iot_catalog.silver.silver_alert_abnormal_state",
