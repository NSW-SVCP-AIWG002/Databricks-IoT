...
6. **Deltaテーブル登録**: 構造化されたセンサーデータをシルバー層のDeltaテーブルに登録
7. **外部DB更新**: デバイスステータス・アラート履歴、シルバー層センサーデータのOLTP DB更新
...
| silver_sensor_data       | シルバー層センサーデータ           |
...
| silver_sensor_data       | シルバー層センサーデータの保持（業種別/顧客作成ダッシュボードが取得する時系列データを格納） |
...
| エラー種別             | 通知タイミング     | 説明                                         |
| ---------------------- | ------------------ | -------------------------------------------- |
| OLTP接続エラー         | 3回リトライ失敗後  | MySQL接続タイムアウト等でOLTP DBに接続不可   |
| キュー書込みエラー     | 3回リトライ失敗後  | メール送信キューへのDelta Lake書込み失敗     |
| Delta Lake書込みエラー | 例外発生時（即時） | センサーデータのDelta Lake書込み時の例外発生 |

詳細は[LDPパイプライン仕様書](./ldp-pipeline-specification.md)のエラーハンドリングセクションを参照。
...
| 2026-04-01 | 1.3  | データ保存先としてOLTP DBを追加 | Kei Sugiyama |
