...
flowchart TB
    ...
    subgraph Output["出力"]
        ...
        AlertState[(Delta Lake<br>silver_alert_abnormal_state)]
        EmailQueue[(Delta Lake<br>silver_email_notification_queue)]
    ...
    Flag --> Update
    Write --> DL
    StateUpdate --> AlertState
    Queue --> EmailQueue
    Update --> OLTP

    AlertState -.->|状態参照| StateCheck

    OLTP -.->|アラート発報/復旧条件参照| Threshold

    EmailQueue -->|1分間隔で取得| BatchJob
    BatchJob --> SMTP
```
...
| カラム名      | データ型  | 説明                                                                   |
| ------------- | --------- | ---------------------------------------------------------------------- |
| key           | BINARY    | メッセージのキー（パーティションの割り当てに使用）                     |
| value         | BINARY    | メッセージの本体（ペイロード）。テレメトリデータなどはここに格納される |
| topic         | STRING    | Event Hubs（Kafkaトピック）の名前                                      |
...
flowchart TD
    ...
    CheckState1 -->|異常中/未発報| ResetState[状態リセット<br>is_abnormal=FALSE]
    CheckState1 -->|正常| NoAction1[処理なし]

    RecoveryProcess --> UpdateHistory[アラート履歴更新<br>復旧日時・ステータス]
    UpdateHistory --> ResetStateWithHistory[状態リセット<br>alert_history_id=NULL]

    Threshold -->|Yes| CheckState2{異常状態<br>テーブル確認}
    CheckState2 -->|正常| StartAbnormal[異常開始記録<br>abnormal_start_time=現在時刻]
    CheckState2 -->|異常中| CheckDuration{継続時間 >=<br>judgment_time?}

    CheckDuration -->|No| UpdateState[状態更新のみ]
    CheckDuration -->|Yes| CheckFired{アラート<br>発報済み?}

    CheckFired -->|Yes| NoAction2[処理なし<br>重複発報防止]
    CheckFired -->|No| FireAlert[アラート発報<br>alert_fired=TRUE]
...
def get_silver_alert_abnormal_state():
    """異常状態テーブルを取得"""
    return spark.read.table("iot_catalog.silver.silver_alert_abnormal_state")
...
```python
def update_silver_alert_abnormal_state(batch_df, batch_id):
    """
    異常状態テーブルの更新（foreachBatchで呼び出し）

    更新パターン:
    1. 新規異常開始: 閾値超過 AND 状態テーブルに未登録または正常状態
       → is_abnormal=TRUE, abnormal_start_time=event_timestamp
    2. 異常継続: 閾値超過 AND 異常状態継続中
       → last_event_time, last_sensor_valueを更新
    3. アラート発報: alert_triggered=TRUE
       → alert_fired=TRUE, alert_fired_time=current_timestamp
       （alert_history_idは別途insert_alert_history()で更新）
    4. 復旧: 閾値正常 AND 異常状態中
       → is_abnormal=FALSE, abnormal_start_time=NULL, alert_fired=FALSE, alert_history_id=NULL
    ...
    # Delta Tableへのマージ
    alert_state_table = DeltaTable.forName(spark, "iot_catalog.silver.silver_alert_abnormal_state")

    alert_state_table.alias("target").merge(
        update_records.alias("source"),
        "target.device_id = source.device_id AND target.alert_id = source.alert_id"
    ).whenMatchedUpdate(
        condition="source.threshold_exceeded = FALSE",
        set={
            # 復旧: 状態リセット（alert_history_idもクリア）
            "is_abnormal": F.lit(False),
            "abnormal_start_time": F.lit(None),
            "last_event_time": F.col("source.event_timestamp"),
            "last_sensor_value": F.col("source.current_sensor_value"),
            "alert_fired": F.lit(False),
            "alert_fired_time": F.lit(None),
            "alert_history_id": F.lit(None),
            "update_time": current_time
        }
    ).whenMatchedUpdate(
        condition="source.threshold_exceeded = TRUE AND target.is_abnormal = FALSE",
        set={
            # 新規異常開始
            "is_abnormal": F.lit(True),
            "abnormal_start_time": F.col("source.event_timestamp"),
            "last_event_time": F.col("source.event_timestamp"),
            "last_sensor_value": F.col("source.current_sensor_value"),
            "alert_fired": F.lit(False),
            "alert_fired_time": F.lit(None),
        ...
        condition="source.threshold_exceeded = TRUE AND target.is_abnormal = TRUE AND source.alert_triggered = TRUE",
        set={
            # アラート発報（alert_history_idは別途更新）
            "last_event_time": F.col("source.event_timestamp"),
            "last_sensor_value": F.col("source.current_sensor_value"),
            "alert_fired": F.lit(True),
            "alert_fired_time": current_time,
            "update_time": current_time
        }
    ).whenMatchedUpdate(
        condition="source.threshold_exceeded = TRUE AND target.is_abnormal = TRUE",
        set={
            # 異常継続（状態更新のみ）
            "last_event_time": F.col("source.event_timestamp"),
            "last_sensor_value": F.col("source.current_sensor_value"),
        ...
        condition="source.threshold_exceeded = TRUE",
        values={
            # 新規レコード挿入（異常開始）
            "device_id": F.col("source.device_id"),
            "alert_id": F.col("source.alert_id"),
            "is_abnormal": F.lit(True),
            "abnormal_start_time": F.col("source.event_timestamp"),
            "last_event_time": F.col("source.event_timestamp"),
            "last_sensor_value": F.col("source.current_sensor_value"),
...
| alert_notification_flag              | 通知を行うか（TRUE/FALSE）                                   |
...
OLTP_RETRY_INTERVALS = [1, 2, 4]        # 指数バックオフ（秒）
...
def get_mysql_connection():
    ...
    for attempt in range(OLTP_MAX_RETRIES):
        try:
...
| リトライ間隔     | 指数バックオフ（1秒、2秒、4秒） | 一時的なネットワーク障害からの復旧を待機                            |
...
def enqueue_email_notification(batch_df, batch_id):
    ...
    # メール送信キューテーブルに書き込み
    queue_records.write.format("delta").mode("append").saveAsTable(
        "iot_catalog.silver.silver_email_notification_queue"
    )
```
...
| 初期ステータス     | PENDING                                         |
| 登録条件           | alert_triggered=TRUE かつ alert_email_flag=TRUE |
| 送信先アドレス取得 | mail_settingマスタから組織IDで取得              |
...
def insert_alert_history(batch_df, batch_id):
    ...
    # 異常状態テーブルにalert_history_idを更新
    if alert_history_updates:
        updates_df = spark.createDataFrame(alert_history_updates)
        alert_state_table = DeltaTable.forName(spark, "iot_catalog.silver.silver_alert_abnormal_state")
        ...
        alert_state_table.alias("target").merge(
            ...
            set={
                "alert_history_id": F.col("source.alert_history_id"),
                "update_time": current_time
            }
        ).execute()
```
...
def update_alert_history_on_recovery(batch_df, batch_id):
    ...
    - 異常状態テーブルでalert_fired = TRUE（アラート発報済み）
    - alert_history_idが存在する
    """
    # 異常状態テーブルから復旧対象を特定
    alert_state_df = spark.read.table("iot_catalog.silver.silver_alert_abnormal_state")
...
| エラー種別             | 例                    | 処理方針                 | リトライ                 |
| ---------------------- | --------------------- | ------------------------ | ------------------------ |
| 入力データエラー       | JSONパース失敗        | レコード破棄             | なし                     |
| スキーマ不整合         | 必須フィールド欠損    | レコード破棄             | なし                     |
| OLTP接続エラー         | MySQL接続タイムアウト | エラーログ記録、処理継続 | 3回                      |
| キュー書込みエラー     | Delta Lake書込み失敗  | エラーログ記録、処理継続 | 3回                      |
| Delta Lake書込みエラー | ストレージエラー      | 例外発生、再処理         | 自動（チェックポイント） |
...
| 監視項目               | 監視間隔     | アラート条件       |
| ---------------------- | ------------ | ------------------ |
| パイプラインステータス | 1分          | 停止・エラー状態   |
| 処理遅延               | 1分          | 60秒超過           |
| エラー率               | 5分          | 1%超過             |
| OLTP接続エラー         | リアルタイム | 3回連続失敗        |
| キュー滞留数           | 5分          | PENDING 1000件超過 |
